    ...
    public class TorWebPageChecker
    {
            ...
            int startupWaitMs = 5000)
        {
            ...
            try
            {
                ...
                bool available = IsTorAvailable(torHost, torPort);
                Console.WriteLine(available
                    ? "[TOR] Started successfully."
                    : "[TOR] Failed to start — .onion links will be skipped (not marked offline).");
        ...
        public async Task<bool> IsOnlineAsync(Uri uri)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[TOR HEAD] {uri} → {statusCode}");
            ...
            {
                Console.WriteLine($"[TOR HEAD failed] {uri}: {ex.Message}");
            }
            ...
            try
            {
                ...
                Console.WriteLine($"[TOR GET] {uri} → {statusCode}");
            ...
            {
                Console.WriteLine($"[TOR GET failed] {uri}: {ex.Message}");
            }

            // No ICMP ping fallback — does not work over Tor
            return false;
        }
    }
}